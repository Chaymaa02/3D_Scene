...
from typing import overload  # useful to check type of arg: scalar or vector?
...
class Trackball_fairy(Trackball):
    def __init__(self):
        super().__init__(yaw=0., roll=0., pitch=0., distance=1., radians=None)
        self.pos2d =  vec(0.0, -0.2)
        self.follow_fairy_z = 0
        self.follow_fairy_x = 0

    def rotate_view_fairy_z (self, angle):
        self.rotation = quaternion_from_euler(0, -angle, 0) 
        
    def view_matrix(self):
        """ View matrix transformation, including distance to target point """
        return translate(*self.pos2d, -self.distance)  @  quaternion_matrix(quaternion_from_euler(0, 0, 8)) @ self.matrix() @ translate(-self.follow_fairy_x,0,-self.follow_fairy_z)
